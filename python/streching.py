...
clrs = [
    ...
    [0.5, '#b10000'],  # Map values between 0 and 0.5 to red
    [0.5, '#00b100'],  # Map values between 0.5 and 1 to green
    [1, '#00b100']  # Map 1 (Yes) to green
]

d = {#'Week_1': [1, 0, 0, 1, 1, 0, 1],
     #'Week of 6/12': week2,
     "Week of 6/19": week3,
     "Week_ of 6/26 ": week4}
     #"": week5}
...
df = pd.DataFrame(d)
fig = make_subplots(rows=2, cols=1, shared_xaxes=True, row_heights=[0.725, 0])

fig.update_layout(yaxis_domain=[0.75, 1], yaxis2_domain=[0, 0.725])  # Assign a smaller y-axis domain for the second heatmap
...
fig.update_layout(
    title_text='Streching',
    title_x=0.5,
    width=799,
    height=400,
    coloraxis=dict(colorscale=clrs, showscale=False),
    yaxis2_tickvals=[0, 1, 2],# hows what title
...
fig.update_traces(xaxis='x2', selector=dict(type='heatmap'))  # Apply xaxis to both heatmaps
